...
## Pre-built drawings (bản vẽ mẫu)

The web workspace ships with ready-made technical drawings loaded from one-click
toolbar buttons (or MCP tools / AI prompts):

| Drawing | Button | What's on the sheet |
|---|---|---|
| Cổng chính 4 cánh (4x3.4m) | `Cổng 4 cánh (4x3.4m)` | Elevation, opening-swing plan, section A-A, materials table |
| Cửa phòng điện 2 cánh (1.7x2.5m) | `Cửa phòng điện (1.7x2.5m)` | Elevation, inward-swing plan, section A-A, safety specs |
| Sơ đồ đi dây & dự toán | `Sơ đồ đi dây + Dự toán` | One-line power diagram, cable routing plan (6 routes), cable schedule, technical notes, cost-estimate summary table, title block |

The wiring sheet is generated by `src/wiringDrawing.ts` from the cost-estimate
module `src/estimate.ts` (single source of truth for the dự toán công trình).
The full estimate can be downloaded as `Du_toan_cong_trinh.md` from the Export
menu, or via API:

- `POST /api/cad/wiring-drawing` — load the wiring + estimate sheet
- `GET /api/estimate` — estimate as JSON
- `GET /api/estimate/export` — full estimate document (Markdown download)
- MCP tools: `wiring_drawing`, `get_estimate`
- Command line: `DODAY` (vẽ sơ đồ đi dây), `DUTOAN` (in bảng dự toán)
- AI prompt: "thiết kế sơ đồ đi dây và dự toán công trình"
